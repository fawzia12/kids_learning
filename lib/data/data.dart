...
const List<LearningItem> learningData = [
  // --- ALPHABET ---
  LearningItem(
      id: 'a1',
      name: 'A',
...
// Category ordering
const List<Category> categoryOrder = [
  Category.alphabet,
  Category.animals,
  Category.seaAnimals,
  Category.fruits,
...
const Map<Category, CategoryMeta> categoryMetadata = {
  Category.alphabet: CategoryMeta(
    title: 'ABC Letters',
    color: 0xFFF9A8D4, // Light Pink
    darkColor: 0xFFDB2777,
    icon: '🔤',
    description: 'Learn A to Z',
  ),
  Category.animals: CategoryMeta(
    title: 'Safari Animals',
    color: 0xFF86EFAC, // Light Green
    darkColor: 0xFF16A34A,
    icon: '🦁',
    description: 'Wild friends',
  ),
  Category.seaAnimals: CategoryMeta(
    title: 'Sea Creatures',
    color: 0xFF93C5FD, // Light Blue
    darkColor: 0xFF2563EB,
    icon: '🐬',
    description: 'Deep blue world',
  ),
  Category.fruits: CategoryMeta(
    title: 'Healthy Fruits',
    color: 0xFFD8B4FE, // Light Purple
    darkColor: 0xFF9333EA,
    icon: '🍎',
...
const List<UnitStepMeta> alphabetSteps = [
  UnitStepMeta(type: StepType.learnUpper, icon: 'A', label: 'Big Letters'),
...
List<UnitStepMeta> getUnitSteps(Category category) {
  return category == Category.alphabet ? alphabetSteps : defaultSteps;
}

List<LearningItem> getItemsByCategory(Category category) {
  return learningData.where((item) => item.category == category).toList();
}

// Zigzag X positions (0.0 - 1.0 fraction of width)
double getZigzagX(int index) {
  switch (index % 7) {
    case 0:
      return 0.5;
    case 1:
